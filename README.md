...
3. Peaks Docker images are stored on Docker Hub (at the moment). On your host machine run the docker container with the command `docker run -it --name my-container-name --ulimit nofile=8192:8192 phyteclabs/chosen_peak:latest` (you may need to use sudo). `chosen_peak` should be the peaks release you want to use(rainier, si, etc.). Note we set the ulimit here because in some instances this has been seen to be very low within a docker container (so we force it here). 
