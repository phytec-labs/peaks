...
3. On your host machine run the docker container with the command `docker run -it --name my-container-name --ulimit nofile=65536:65536 docker.pkg.github.com/phytec-labs/peaks/chosen_peak:latest` (you may need to use sudo). `chosen_peak` should be the peaks release you want to use(rainier, si, etc.). Note we set the ulimit here because in some instances this has been seen to be very low within a docker container (so we force it here). 
